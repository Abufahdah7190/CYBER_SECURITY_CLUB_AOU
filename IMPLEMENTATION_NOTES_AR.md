...
أصبح `npm start` يطبق جميع ملفات `server/db/migrations/` تلقائيًا وبترتيبها قبل تشغيل التطبيق، مع تتبع التنفيذ في جدول `schema_migrations`. لذلك لا يعتمد نشر Render على خطوة يدوية منفصلة للـ migrations.
...
## مراجعة نظام الشهادات — 2026-09-06

- تم توحيد الشهادة على قالب **Light** فقط ومطابقة الإطار المرجعي الأزرق/البنفسجي النظيف.
- تم إضافة شعار هندسي مفرغ شفاف بدون النص الداخلي، مع عزل موضعه أعلى الشهادة لمنع أي تداخل.
- تم تثبيت QR Code وبيانات رمز التحقق في بطاقة واضحة بالزاوية السفلى اليمنى.
- تم إصلاح صفحة التحقق لتستخدم `imageUrl` المُولّد من الخادم مع cache-busting، بدل عنوان صورة ثابت قد يعرض نسخة قديمة.
- تم تفعيل تشغيل migrations تلقائيًا قبل بدء السيرفر؛ وهذا يعالج حالة تشغيل الكود قبل إضافة عمود `theme` وما ينتج عنها من أخطاء 500 في مسارات الشهادات.
- تم اجتياز فحص الشهادة الداخلي للعربية والإنجليزية (سلامة SVG، `lang`، النص الخاص باللغة، ووجود الشعار المضمّن).
- تم اجتياز `node --check` لملفات خدمة الشهادات، migration runner، مسارات التعلم، التطبيق، وملف تشغيل الخادم.
- لم يمكن تنفيذ اتصال PostgreSQL حقيقي من بيئة الفحص الحالية لأن ملف المشروع لا يحتوي بيانات قاعدة بيانات تشغيلية، كما أن `node_modules` المرفقة كانت ناقصة (`pg` و`qrcode` غير متاحة محليًا). لذلك اختبار migration ضد قاعدة فعلية وHTTP Status 500 على Render يحتاج تشغيل النسخة المنشورة ببيانات Supabase/Render الفعلية.
